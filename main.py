...
async def on_startup():
    ...
    await bot.api.messages.send(peer_id=486697492, message=form, attachment=photo,
                                keyboard=Keyboard(inline=True).add(Callback("Принять", {'form_accept': 28}), KeyboardButtonColor.POSITIVE)
                                .row().add(Callback('Отклонить', {'form_decline': 28}), KeyboardButtonColor.NEGATIVE))
