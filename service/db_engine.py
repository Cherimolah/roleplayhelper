...
class Database(Gino):
    ...
    def __init__(self, *args, **kwargs):
        ...
        class Form(self.Model):
            ...
            # Засекреченные блоки анкеты (см. module classified_profiles). Видны владельцу анкеты,
            # администраторам и игрокам с допуском (см. classified_profession_id / classified_fraction_id + classified_reputation)
            secret_features = Column(Text, nullable=True)  # Засекреченные физиологические особенности
            secret_bio = Column(Text, nullable=True)  # Засекреченная биография
            secret_character = Column(Text, nullable=True)  # Засекреченный характер
            secret_motives = Column(Text, nullable=True)  # Засекреченные мотивы нахождения на станции
            classified_profession_id = Column(Integer, ForeignKey("professions.id", ondelete='SET NULL'), nullable=True)  # Допуск по профессии
            classified_fraction_id = Column(Integer, ForeignKey("fractions.id", ondelete='SET NULL'), nullable=True)  # Допуск по фракции
            classified_reputation = Column(Integer, nullable=True)  # Необходимый уровень репутации в classified_fraction_id для допуска
        ...
        class Quest(self.Model):
            ...
            reward = Column(JSON)  # Объект награды (см. README)
            penalty = Column(JSON)  # Объект штрафа (см. README)
            force_assign_forms = Column(ARRAY(Integer), nullable=True)  # Айди анкет, которым нужно принудительно выдать квест сразу после создания (см. [выдать задачу ...], module admin_improvements п.9)
        ...
        class StateDebuff(self.Model):
            ...
            action_time = Column(Integer, default=0)  # Количество циклов, которое действует дебаф в эушен-режиме
            time_use = Column(Integer, default=0)  # Время действия дебафа (в секундах)
            pov_effect = Column(Text, nullable=True)  # Ключ эффекта из service.effects.EFFECTS, который дебаф принудительно включает игроку в POV-режиме (см. service/pov_debuffs.py)
        ...
        class Chat(self.Model):
            ...
            is_private = Column(Boolean, default=False)  # Является ли он приватным
            visible_messages = Column(Integer, default=1000)  # Сколько сообщений видно при приглашении / ссылке
            cabin_user_id = Column(Integer, ForeignKey('users.user_id', ondelete='SET NULL'))  # Владелец каюты (если это чат каюты)
            user_chat_id = Column(Integer)  # Айди чата для юзер-бота
            cabin_number = Column(Integer, nullable=True)  # Номер каюты (сохраняется и после архивации/освобождения, чтобы не плодить дубликаты чатов для того же номера)
        ...
        class Auction(self.Model):
            """
            Таблица с аукционами (module auction_system). Лотом аукциона является предмет карты
            экспедитора (db.Item) — у него уже есть фото/описание для поста и fraction_id/reputation
            для фильтрации получателей закрытого аукциона.
            """
            __tablename__ = 'auctions'

            id = Column(Integer, primary_key=True)
            item_id = Column(Integer, ForeignKey('items.id', ondelete='CASCADE'))  # Лот аукциона
            is_public = Column(Boolean, default=True)  # Публичный (стена + комментарии) или закрытый (ЛС)
            start_at = Column(TIMESTAMP)  # Время начала (публикации / рассылки)
            end_at = Column(TIMESTAMP)  # Время завершения
            start_price = Column(Integer, default=0)  # Стартовая цена
            min_bid_step = Column(Integer, default=1)  # Минимальный шаг ставки
            wall_post_id = Column(Integer, nullable=True)  # Айди поста на стене (для публичного аукциона)
            is_started = Column(Boolean, default=False)  # Уже опубликован/разослан
            is_finished = Column(Boolean, default=False)  # Уже подведены итоги
            winner_user_id = Column(Integer, ForeignKey('users.user_id', ondelete='SET NULL'), nullable=True)
            winner_bid = Column(Integer, nullable=True)
            created_by = Column(Integer, ForeignKey('users.user_id', ondelete='SET NULL'))

        self.Auction = Auction

        class AuctionBid(self.Model):
            """
            Таблица со ставками участников аукциона.
            Для публичного аукциона привязана к конкретному комментарию на стене (защита от дублей).
            """
            ...
            comment_id = Column(Integer, nullable=True)  # Айди комментария на стене (для публичного аукциона)
            created_at = Column(DateTime(timezone=True), default=now)

        self.AuctionBid = AuctionBid

        class StealthLog(self.Model):
            """
            Журнал скрытных действий (см. sub_module stealth_mode).
            Пересылается только судьям/администраторам при успешной проверке.
            """
            __tablename__ = 'stealth_logs'

            id = Column(Integer, primary_key=True)
            user_id = Column(Integer, ForeignKey('users.user_id', ondelete='CASCADE'))  # Кто совершил действие
            chat_id = Column(Integer, ForeignKey('chats.chat_id', ondelete='CASCADE'))  # В какой локации
            text = Column(Text)  # Текст скрытного действия
            success = Column(Boolean, default=False)  # Удалось ли скрыться
            created_at = Column(DateTime(timezone=True), default=now)

        self.StealthLog = StealthLog
