...
async def create_action_mode_request(m: Message):
    ...
    chat_name = (await bot.api.messages.get_conversations_by_id(peer_id=m.peer_id)).items[0].chat_settings.title
    judges = [x[0] for x in await db.select([db.User.user_id]).where(db.User.judge.is_(True)).gino.all()]
    for judge_id in judges:
        request = await db.ActionModeRequest.create(judge_id=judge_id, chat_id=m.chat_id)
        keyboard = Keyboard(inline=True).add(
            Callback('Принять', {'action_mode_request_id': request.id, 'action': 'confirm'}), KeyboardButtonColor.POSITIVE
        ).row().add(
            Callback('Отклонить', {'action_mode_request_id': request.id, 'action': 'decline'}), KeyboardButtonColor.POSITIVE
        )
