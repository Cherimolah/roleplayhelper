"""
Модуль проверки действий игроков в экшен-режиме.
Содержит логику взаимодействия судьи с системой проверки постов,
включая установку сложности, бонусов и последствий действий.
"""
...
from service.utils import next_step, show_consequences, serialize_consequence, type_difficulties, count_difficult, \
    count_attribute, apply_consequences, apply_item, create_mention, get_current_form_id


async def send_check(m: Message | MessageEvent, post_id: int):
    """
    Основная функция проверки поста. Управляет процессом проверки действий,
    включая применение предметов и проверку стандартных действий.
    """
    ...
    await db.ActionMode.update.values(number_check=db.ActionMode.number_check + 1).where(
        db.ActionMode.id == action_mode_id).gino.status()
    ...
    # Проверяем завершение проверки или применение предмета
    if len(actions) < number_check or number_check > 5:
        if actions[-1].data['type'] == 'use_item':
            await db.ActionMode.update.values(check_status=False).where(
                db.ActionMode.id == action_mode_id).gino.status()
            ...
            if not finished:
                ...
                if isinstance(m, Message):
                    ...
                    await bot.api.messages.send(peer_id=judge_id, message='Основное меню',
    ...
    # Получаем текущее действие для проверки
    action = actions[number_check - 1]

    # Обрабатываем применение предмета
    ...
    # Подготавливаем текст действия для отображения
    ...
    # Устанавливаем состояние для ввода бонуса/штрафа
    ...
    if isinstance(m, Message):
        ...
        await db.User.update.values(state=f'{Judge.SET_BONUS}*{action.id}').where(
            db.User.user_id == judge_id).gino.status()
...
async def start_check(m: MessageEvent):
    """Начинает проверку поста игрока"""
    ...
    # Помечаем пост как начатый для проверки
    ...
    # Переходим к установке сложности
...
async def set_difficult(m: Message):
    """Устанавливает базовую сложность проверки поста"""
    ...
    # Переходим к установке возможности отказа от проверки
...
async def set_bonus_check(m: Message, value: int):
    """Устанавливает бонус/штраф судьи для текущего действия"""
    ...
    # Переходим к выбору атрибута для проверки
    ...
    # Создаем клавиатуру с атрибутами
    keyboard = Keyboard()
...
async def set_attribute(m: Message):
    """Устанавливает атрибут для проверки и переходит к настройке последствий"""
    ...
    # Создаем соответствующую клавиатуру последствий
    if data['type'] == 'action':
...
@bot.on.private_message(StateRule(Judge.DELETE_CONSEQUENCES), PayloadRule({'set_consequence_type': 'back'}),
                        JudgeRule())
async def select_consequence_type(m: Message):
    """Обрабатывает выбор типа последствия или возврат к предыдущему шагу"""
...
async def select_group_consequence(m: Message):
    """Обрабатывает выбор группы последствий"""
    ...
    # Обрабатываем специальную группу "Другое"
    if group_id != 5:
...
async def set_other_consequence(m: Message):
    """Устанавливает текстовое последствие"""
    _, action_id, con_var, group_id, _ = states.get(m.from_id).split('*')
...
async def select_type_consequences(m: Message):
    """Обрабатывает выбор конкретного типа последствия"""
    ...
    # Получаем ID экспедитора в зависимости от типа действия
    if con_var <= 4:
    ...
    # Обрабатываем различные типы последствий
    match con_type:
        case 1:  # Получение травмы
            injuries = [x[0] for x in
                        await db.select([db.StateDebuff.name]).where(db.StateDebuff.type_id == 1).order_by(
        ...
        case 5:  # Снятие всех травм
            await db.Consequence.create(action_id=action_id, data={'type': 'delete_debuff_type', 'debuff_type_id': 1},
                                        type=con_var)
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
            return
        case 6:  # Снятие всех безумий
            await db.Consequence.create(action_id=action_id, data={'type': 'delete_debuff_type', 'debuff_type_id': 2},
                                        type=con_var)
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
            return
        ...
        case 14:  # Отключение предмета
            ...
            await m.answer(reply, keyboard=keyboard)
            return
        case x:  # Изменение характеристик (атрибутов)
...
async def set_consequences_data(m: Message):
    """Устанавливает данные для выбранного типа последствия"""
    ...
    # Проверяем необходимость числового ввода
    ...
    match con_type:
        case 1:  # Получение травмы
            ...
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'add_debuff', 'debuff_id': debuff_id})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
            return
        case 2:  # Получение безумия
        ...
        case 3:  # Снятие травмы
            ...
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'delete_debuff', 'row_id': row_id})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case 4:  # Снятие безумия
            injuries = [x[0] for x in await db.select([db.StateDebuff.id]).select_from(
        ...
        case 9:  # Изменение подчинения
            if not -100 <= number <= 100:
                ...
                return
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'add_subordination', 'bonus': number})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case 10:  # Установить оплодотворение
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'set_pregnant', 'text': m.text})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case 12:  # Получение предмета
            ...
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'add_item', 'item_id': item_id})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case 13:  # Удаление предмета
            ...
            item_id = items[number - 1]
            await db.Consequence.create(action_id=action_id, type=con_var,
                                        data={'type': 'delete_item', 'row_id': item_id})
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case 14:  # Отключение предмета
            items = [x[0] for x in await db.select([db.Item.id]).select_from(
            ...
            await m.answer('Последствие успешно добавлено')
            await select_group_consequence(m)
        case x:  # Изменение характеристик
            attribute_id = await db.select([db.Attribute.id]).where(db.Attribute.id == x - 15).gino.scalar()
            if not attribute_id:
                return
            await db.Consequence.create(action_id=action_id, type=con_var,
...
async def select_to_delete_consequences(m: Message):
    """Показывает список последствий для удаления"""
    _, action_id, con_var = states.get(m.from_id).split('*')
    action_id = int(action_id)
    con_var = int(con_var)

    con_data = [x[0] for x in await db.select([db.Consequence.data]).where(
        and_(db.Consequence.action_id == action_id, db.Consequence.type == con_var)).order_by(
        db.Consequence.id.asc()).gino.all()]
...
async def delete_consequence(m: Message, value: int):
    """Удаляет выбранное последствие"""
    _, action_id, con_var = states.get(m.from_id).split('*')
    action_id = int(action_id)
    con_var = int(con_var)

    con_id = await db.select([db.Consequence.id]).where(
        and_(db.Consequence.action_id == action_id, db.Consequence.type == con_var)).order_by(
        db.Consequence.id.asc()).offset(value - 1).limit(1).gino.scalar()
    if not con_id:
        await m.answer('Неправильно указан номер!')
        return
...
async def finish_action_check(m: Message):
    """Завершает проверку действия и отправляет результат игроку"""
    action_id = int(states.get(m.from_id).split('*')[1])
    post_id = await db.select([db.Action.post_id]).where(db.Action.id == action_id).gino.scalar()
    can_decline, user_id, diffucult = await db.select(
        [db.Post.decline_check, db.Post.user_id, db.Post.difficult]).where(db.Post.id == post_id).gino.first()

    # Создаем клавиатуру для игрока
    keyboard = Keyboard(inline=True).add(
        Callback('Принять проверку', {'action_check': 'accept', 'action_id': action_id}), KeyboardButtonColor.POSITIVE
    )
    if can_decline:
        keyboard.row().add(
            Callback('Отказаться от проверки', {'action_check': 'decline', 'action_id': action_id}),
    ...
    # Формируем сообщение с информацией о проверке
    ...
    # Определяем уровень бонуса/штрафа
    if action.bonus == 0:
        value = 'Отсутсвует'
...
@bot.on.raw_event(GroupEventType.MESSAGE_EVENT, MessageEvent,
                  PayloadMapRule({'action_check': 'decline', 'action_id': int}))
async def decline_check(m: MessageEvent):
    """Обрабатывает отказ игрока от проверки"""
    ...
    await bot.api.messages.send(peer_id=2000000000 + chat_id,
                                message=f'Игрок [id{m.user_id}|{name} / {user.first_name} {user.last_name}] '
...
@bot.on.raw_event(GroupEventType.MESSAGE_EVENT, MessageEvent,
                  PayloadMapRule({'action_check': 'accept', 'action_id': int}))
async def accept_check(m: MessageEvent):
    """Обрабатывает принятие игроком проверки и применяет последствия"""
    action_id = m.payload['action_id']
    action = await db.Action.get(action_id)
    ...
    target_percentage = ((await count_attribute(m.user_id, action.attribute_id)) + action.bonus) * \
                        type_difficulties[difficult][1]

    if action.data.get('type') == 'action':
        # Генерируем случайный результат для одиночного действия
        ...
        # Вычисляем результат для PvP действия
        opponent_percentage = ((await count_attribute(action.data.get('user_id'),
                                                      action.attribute_id)) + action.bonus) * \
                              type_difficulties[difficult][1]
    ...
    # Обновляем статус проверки и продолжаем проверку
    action_mode_id = await db.select([db.Post.action_mode_id]).where(db.Post.id == action.post_id).gino.scalar()
    await db.ActionMode.update.values(check_status=False).where(db.ActionMode.id == action_mode_id).gino.status()
