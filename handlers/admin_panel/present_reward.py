"""
Модуль для выдачи наград и штрафов пользователям.
Позволяет администраторам начислять или списывать средства с баланса пользователей.
"""
...
async def give_reward(m: Message):
    """Начало процесса выдачи награды/штрафа"""
...
async def user_reward(m: Message, forms: List[Tuple[int, int]] = None, form: Tuple[int, int] = None):
    """Обработка выбора пользователя для награды/штрафа"""
    if forms:
...
async def set_amount_reward(m: Message, forms: List[Tuple[int, int]]):
    """Установка суммы награды/штрафа"""
    try:
        amount = int(m.text)
    except ValueError:
...
async def accept_reward(m: Message, forms: List[Tuple[int, int]]):
    """Подтверждение и применение награды/штрафа"""
    form_ids = [x[0] for x in forms]
    user_ids = [x[1] for x in forms]
