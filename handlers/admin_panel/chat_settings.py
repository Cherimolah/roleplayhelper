"""
Модуль для управления настройками чатов.
Содержит обработчики для изменения типа чата, количества видимых сообщений,
управления доступными профессиями и другие административные функции.
"""
...
@bot.on.private_message(StateRule(Admin.CHAT_SET_VISIBLE_MESSAGES_COUNT),
                        PayloadRule({'set_visible_messages_count': 'back'}))
@bot.on.private_message(StateRule(Admin.CHAT_SET_VISIBLE_MESSAGES_COUNT),
                        PayloadRule({'set_visible_messages_count': 'back'}))
...
async def chat_settings(m: Message):
    """
    Основной обработчик настроек чата.

    Args:
        m: Сообщение от пользователя

    Функционал:
        - Регистрация нового чата при первом обращении
        - Проверка прав администратора
        - Отправка текущих настроек чата
        - Переход в ЛС для дальнейшей настройки
    """
    if m.peer_id > 2000000000:  # Проверка что сообщение из чата
        ...
        if not registered:
            try:
                ...
                return
            try:
                # Проверяем доступ к пригласительной ссылке
...
async def change_chat_type(m: Message):
    """Обработчик изменения типа чата (приватный/публичный)"""
...
async def set_chat_type(m: Message):
    """Установка выбранного типа чата"""
    ...
    if is_private:
        # Для приватного чата проверяем наличие user-бота
        ...
        if not user_chat_id:
            ...
            # Отправляем команду для установки связи
            message = (await bot.api.messages.send(message=f'/приват {chat_id}', peer_id=chat_id + 2000000000))[0]
            await asyncio.sleep(3)
            await bot.api.messages.delete(cmids=[message.conversation_message_id], peer_id=chat_id + 2000000000,
                                          delete_for_all=True)
            user_chat_id = await db.select([db.Chat.user_chat_id]).where(db.Chat.chat_id == chat_id).gino.scalar()
            if not user_chat_id:
                await m.answer(
                    'Не удалось установить связь между аккаунтом и ботом. Возможно юзер-бот не имеет права приглашать пользователей')
                return

    # Обновляем тип чата в базе данных
...
async def connect_private_chat(m: Message, chat_id: int):
    """Обработчик для user-бота: установка связи с приватным чатом"""
    can_invite = (await user_bot.api.messages.get_conversations_by_id(peer_ids=[m.peer_id])).items[
...
@bot.on.private_message(PayloadRule({'chat_settings': 'visible_messages_count'}), StateRule(Admin.CHAT_SETTINGS),
                        AdminRule())
async def send_info_visible_messages_count(m: Message):
    """Обработчик для изменения количества видимых сообщений"""
    _, chat_id = states.get(m.from_id).split('*')
...
@bot.on.private_message(StateRule(Admin.CHAT_SET_VISIBLE_MESSAGES_COUNT), NumericRule(max_number=1000, min_number=0),
                        AdminRule())
async def set_visible_messages_count(m: Message, value: int):
    """Установка количества видимых сообщений"""
    _, chat_id = states.get(m.from_id).split('*')
    chat_id = int(chat_id)
...
async def show_available_professions(m: Message):
    """Показ доступных для добавления профессий"""
    _, chat_id = states.get(m.from_id).split('*')
    chat_id = int(chat_id)
    # Получаем ID уже активных профессий
    active_profession_ids = [x[0] for x in await db.select([db.ChatToProfessions.profession_id]).where(
        db.ChatToProfessions.chat_id == chat_id).gino.all()]
    # Получаем названия неактивных профессий
    names = [x[0] for x in
             await db.select([db.Profession.name]).where(db.Profession.id.notin_(active_profession_ids)).order_by(
...
async def open_profession(m: Message, value: int):
    """Добавление профессии в доступные для чата"""
    _, chat_id = states.get(m.from_id).split('*')
    chat_id = int(chat_id)
    active_profession_ids = [x[0] for x in await db.select([db.ChatToProfessions.profession_id]).where(
        db.ChatToProfessions.chat_id == chat_id).gino.all()]
    # Получаем ID профессии по номеру
    profession_id = await db.select([db.Profession.id]).where(db.Profession.id.notin_(active_profession_ids)).order_by(
        db.Profession.id.asc()).offset(value - 1).limit(1).gino.scalar()
    if not profession_id:
        await m.answer('Неверный номер')
        return
    # Добавляем профессию в доступные
    await db.ChatToProfessions.create(chat_id=chat_id, profession_id=profession_id)
    await m.answer('Значение успешно установлено')
    await chat_settings(m)


@bot.on.private_message(StateRule(Admin.CHAT_SETTINGS), PayloadRule({'chat_settings': 'delete_available_professions'}),
                        AdminRule())
async def select_delete_profession(m: Message):
    """Показ профессий для удаления из доступных"""
    _, chat_id = states.get(m.from_id).split('*')
    chat_id = int(chat_id)
    names = [x[0] for x in await db.select([db.Profession.name]).select_from(
        db.ChatToProfessions.join(db.Profession, db.ChatToProfessions.profession_id == db.Profession.id)
...
async def delete_profession(m: Message, value: int):
    """Удаление профессии из доступных для чата"""
    _, chat_id = states.get(m.from_id).split('*')
    chat_id = int(chat_id)
    # Получаем ID записи для удаления
    row_id = await db.select([db.ChatToProfessions.id]).where(db.ChatToProfessions.chat_id == chat_id).order_by(
        db.ChatToProfessions.id.asc()).offset(value - 1).limit(1).gino.scalar()
    if not row_id:
        await m.answer('Неверный номер')
        return
    # Удаляем профессию из доступных
...
async def save_profession(m: Message):
    """Сохранение настроек и возврат в главное меню"""
    await start(m)
...
async def set_cabin_number(m: Message, cabin_number: int):
    ...
    # Устанавливаем связь с user-ботом
