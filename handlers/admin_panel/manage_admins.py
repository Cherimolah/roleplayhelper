...
async def enter_new_admin_id(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.enter_new_admin_id)
...
async def ask_new_admin(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.confirm_new_admin.format(f"[id{user[0].id}|{user[0].first_name} {user[0].last_name}]"),
                        keyboard=keyboard)
...
async def cancel_new_admin(m: Message):
    states.set(m.from_id, Admin.ENTER_NEW_ADMIN_ID)
    await bot.write_msg(m.peer_id, messages.enter_new_admin_id, keyboard=keyboards.manage_admins)
...
async def add_new_admin(m: Message):
    user_id = json.loads(m.payload)['new_admin']
    ...
    user = await bot.api.users.get(user_id)
    await bot.write_msg(m.peer_id, messages.new_admin.format(f"[id{user[0].id}|{user[0].first_name} {user[0].last_name}]"))
...
async def delete_old_admin(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.enter_old_admin)
...
async def ask_old_admin(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.confirm_old_admin.format(f"[id{user[0].id}|{user[0].first_name} {user[0].last_name}]"),
                        keyboard=keyboard)
...
async def cancel_old_admin(m: Message):
    states.set(m.from_id, Admin.ENTER_OLD_ADMIN_ID)
    await bot.write_msg(m.peer_id, messages.enter_old_admin, keyboard=keyboards.manage_admins)
...
async def delete_old_admin_(m: Message):
    user_id = json.loads(m.payload)['old_admin']
    await db.User.update.values(admin=0).where(db.User.user_id == int(user_id)).gino.status()
    states.set(m.from_id, Admin.SELECT_MANAGE_ADMINS)
    user = await bot.api.users.get(user_id)
    await bot.write_msg(m.peer_id, messages.deleted_admin.format(f"[id{user[0].id}|{user[0].first_name} {user[0].last_name}]"))
    await bot.write_msg(m.peer_id, messages.manage_admins, keyboard=keyboards.manage_admins)
