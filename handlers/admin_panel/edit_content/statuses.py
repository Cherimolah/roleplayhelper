...
async def start_create_new_atatus(m: Message):
    ...
    await bot.write_msg(m.peer_id, "Введите название статуса", keyboard=Keyboard())
...
async def new_status(m: Message):
    ...
    await bot.write_msg(m.peer_id, "Статус успешно создан", keyboard=keyboards.gen_type_change_content("statuses"))
...
async def delete_status(m: Message, value: int = None):
    ...
    states.set(m.from_id, Admin.SELECT_ACTION)
    await bot.write_msg(m.peer_id, "Статус успешно удалён", keyboard=keyboards.gen_type_change_content("statuses"))
