"""
Модуль банковских операций бота.
Обрабатывает финансовые операции: переводы, баланс, история транзакций, зарплаты, пожертвования.
"""
...
async def send_create_transactions(m: Message):
    """Начать процесс перевода - показать список пользователей"""
    ...
    # Создание инлайн-клавиатуры для пагинации если пользователей больше 15
    keyboard = None
    if count_forms > 15:
...
async def send_page(m: MessageEvent):
    """Обработка пагинации списка пользователей для перевода"""
...
async def select_user_to_transfer(m: Message):
    """Выбор пользователя для перевода по номеру или упоминанию"""
    if m.text.isdigit():
        ...
        # Выбор по упоминанию пользователя
...
async def set_amount_transfer(m: Message, value: int = None):
    """Установка суммы перевода с расчетом комиссии"""
    if value < 1:
        await m.answer(messages.error_small_amount)
        return

    # Расчет комиссии и налога
    ...
    # Подготовка данных для подтверждения перевода
...
async def confirm_transaction(m: Message):
    ...
    await bot.api.messages.send(peer_id=user_id, message=messages.trasaction_catch.format(name, amount, from_name),
...
async def ask_salary(m: Message):
    ...
    name, profession_id = await db.select([db.Form.name, db.Form.profession]).where(
        db.Form.user_id == m.from_id).gino.first()
...
async def send_permanent_costs(m: Message):
    """Показать постоянные расходы пользователя"""
    ...
    # Расчет стоимости функционального декора
...
async def show_piggy_bank(m: Message):
    ...
    # Создание клавиатуры пагинации если пожертвований больше 15
    kb = None
    if count > 15:
    ...
    # Формирование списка пожертвований
    pages = count // 15
    if count % 15 != 0:
        pages += 1
...
async def send_donate_page(m: MessageEvent):
    """Обработка пагинации списка пожертвований"""
    ...
    # Формирование списка пожертвований для текущей страницы
    reply = f"Список пожертвований:\nСтраница {page}/{pages}\n\n"
    for i, donate in enumerate(donates):
        amount, form_id, name, user_id = donate
...
async def create_donate(m: Message):
    """Начать процесс создания пожертвования"""
...
async def enter_amount_donate(m: Message, value: int):
    ...
    # Клавиатура подтверждения пожертвования
    kb = Keyboard()
