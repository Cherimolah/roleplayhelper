...
async def send_bank_menu(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.bank_menu, keyboard=keyboards.bank)
...
async def send_balance(m: Message):
    ...
    await bot.write_msg(m.peer_id, messages.balance.format(balance))
...
async def send_transfer_history(m: Message):
    ...
    if not transactions:
        await bot.write_msg(m.peer_id, messages.not_transactions)
        return
    reply = ""
    ...
    await bot.write_msg(m.peer_id, messages.history_transactions.format(reply))
...
async def send_create_transactions(m: Message):
    form_id = await db.select([db.Form.id]).select_from(
    ...
    if not forms:
        await bot.write_msg(m.peer_id, "Пока анкет нет")
        return
    ...
    await bot.write_msg(m.peer_id, messages.select_users_to_transfers, keyboard=keyboard)
    keyboard = None
    if count_forms > 15:
...
async def send_page(m: MessageEvent):
    ...
    await bot.edit_msg(m, reply, keyboard=keyboard.get_json())
...
async def select_user_to_transfer(m: Message):
    ...
    if not form_id:
        await bot.write_msg(m.peer_id, messages.error_user_to_transfer)
        return
    states.set(m.from_id, f"{Menu.SELECT_AMOUNT_TO_TRANSFER}@{form_id}")
    await bot.write_msg(m.peer_id, messages.amount_transfer)
...
async def set_amount_transfer(m: Message, value: int = None):
    if value < 1:
        await bot.write_msg(m.peer_id, messages.error_small_amount)
        return
    if value % 2 == 0:
        commission = value // 2
    else:
    ...
    if balance < value + tax:
        states.set(m.from_id, Menu.BANK_MENU)
        await bot.write_msg(m.peer_id, messages.error_not_enogh_tokens.format(balance, value + tax),
    ...
    await bot.write_msg(m.peer_id, messages.confirm_transfer.format(f"[id{user_id}|{name}]", value, value + tax, tax),
                        keyboard=keyboard)
...
async def confirm_transaction(m: Message):
    ...
    current_form_id = await db.select([db.Form.id]).select_from(
        db.Form.join(db.User, and_(db.Form.user_id == db.User.user_id, db.Form.number == db.User.activated_form))
    ).where(db.Form.user_id == m.from_id).gino.scalar()
    ...
    from_name = await db.select([db.Form.name]).select_from(
        db.Form.join(db.User, and_(db.User.activated_form == db.Form.number))
    ).where(db.Form.user_id == m.from_id).gino.scalar()
    from_name = f"[id{m.from_id}|{from_name}]"
    await bot.write_msg(user_id, messages.trasaction_catch.format(name, amount, from_name))
    states.set(m.from_id, Menu.BANK_MENU)
    await bot.write_msg(m.peer_id, messages.transfer_success, keyboard=keyboards.bank)
...
async def send_permanent_costs(m: Message):
    cabin_lux = await db.select([db.Form.cabin_type]).select_from(
    ...
    await bot.write_msg(m.peer_id, messages.permament_costs.format(price))
...
async def show_piggy_bank(m: Message):
    ...
    if summary is None:
        summary = 0
    await bot.write_msg(m.peer_id, f"Всего пожертвовано: {summary}", keyboard=keyboards.donate_menu)
    ...
    await bot.write_msg(m.peer_id, reply, keyboard=kb)
...
async def send_donate_page(m: MessageEvent):
    ...
    if count - page * 15 > 0:
        kb.add(Callback("->", {"donate_page": page + 1}), KeyboardButtonColor.PRIMARY)
    await bot.edit_msg(m, reply, keyboard=kb)
...
async def create_donate(m: Message):
    ...
    await bot.write_msg(m.peer_id, "Введите сумму для пожертвования", keyboard=kb)
...
async def enter_amount_donate(m: Message, value: int):
    ...
    if balance < value:
        await bot.write_msg(m.peer_id, "На балансе недостаточно средств!")
        states.set(m.from_id, Menu.BANK_MENU)
        await bot.write_msg(m.peer_id, messages.bank_menu, keyboard=keyboards.bank)
        return
    ...
    kb.row()
    kb.add(Text("Назад", {"bank_menu": "donate"}), KeyboardButtonColor.NEGATIVE)
    await bot.write_msg(m.peer_id, f"Подтверждаете пожертвование в храм в сумме {value}?", keyboard=kb)
...
async def confirm_donate(m: Message):
    ...
    form_id = await (db.select([db.Form.id])
                     .select_from(db.User.join(db.Form,
                                               and_(db.Form.user_id == db.User.user_id,
                                                    db.User.activated_form == db.Form.number)))
                     .where(db.Form.user_id == m.from_id).gino.scalar())
    ...
    await bot.write_msg(m.peer_id, f"Успешно пожертвовано в храм {amount} монет", keyboard=keyboards.bank)
