...
async def daughter_quest(m: Message):
    ...
    confirmed_target_ids = {x[0] for x in await db.select([db.DaughterTargetRequest.target_id]).where(
        and_(db.DaughterTargetRequest.confirmed.is_(True), db.DaughterTargetRequest.created_at == datetime.date.today(),
    ...
    for i, target_id in enumerate(target_ids):
        confirmed = await db.select([db.DaughterTargetRequest.confirmed]).where(
            and_(db.DaughterTargetRequest.form_id == form_id, db.DaughterTargetRequest.created_at == datetime.date.today(),
        ...
        if confirmed is None:
            emoji = '❌'
            keyboard.add(Text(f'Завершить доп. цель №{i + 1}', {'daughter_target_complete': target_id}), KeyboardButtonColor.PRIMARY).row()
    ...
    quest_request = await db.select([db.DaughterQuestRequest.confirmed]).where(
        and_(db.DaughterQuestRequest.form_id == form_id, db.DaughterQuestRequest.quest_id == quest.id,
...
async def confirm_daughter_target(m: Message):
    ...
    keyboard = Keyboard(inline=True).add(Callback('Подтвердить', {'confirm_daughter_target_request': request.id}), KeyboardButtonColor.POSITIVE).row()
    keyboard.add(Callback('Отклонить', {'decline_daughter_target_request': request.id}), KeyboardButtonColor.NEGATIVE)
...
async def confirm_daughter_target(m: Message):
    ...
    name = await db.select([db.Form.name]).where(db.Form.id == form_id).gino.scalar()
    user = (await bot.api.users.get(user_ids=m.from_id))[0]
    keyboard = Keyboard(inline=True).add(Callback('Подтвердить', {'confirm_daughter_quest_request': request.id}), KeyboardButtonColor.POSITIVE).row()
    keyboard.add(Callback('Отклонить', {'decline_daughter_quest_request': request.id}), KeyboardButtonColor.NEGATIVE)
    await bot.api.messages.send(peer_ids=admins,
                                message=f'Отчёт о выполнении дочери [id{m.from_id}|{name} / '
