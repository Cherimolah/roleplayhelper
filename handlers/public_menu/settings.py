"""
Модуль настроек пользователя и администратора:
- Управление уведомлениями
- Запросы на заморозку/разморозку анкеты
- Запросы на удаление анкеты
- Настройки таймеров (для администраторов)
- Перезапуск бота (для администраторов)
"""
...
async def send_freeze_request(m: Message):
    """Обработчик запроса на заморозку/разморозку анкеты"""
    ...
    kb = Keyboard().add(
        Text(f"Подтвердить", {"freeze": "send_request"} if not freeze else {"unfreeze": "send_request"}),
    ...
    await m.answer(
        'Вы действительно готовы "разморозить" свою анкету, чтобы вернуться к активной деятельности на станции?'
        if freeze else 'Вы действительно хотите оформить запрос для отправки вашей анкеты "на мороз"?',
        keyboard=kb)
...
async def ask_reason_freeze(m: Message):
    """Запрос причины заморозки анкеты"""
...
async def freeze_request_send_accepted(m: Message):
    """Отправка запроса на заморозку/разморозку администраторам"""
    ...
    kb = Keyboard(inline=True).add(
        Callback(f"{'Разморозить' if freeze else 'Заморозить'}", {"freeze": "accept", "user_id": m.from_id}),
        KeyboardButtonColor.POSITIVE
    ).add(
...
async def ask_delete(m: Message):
    """Обработчик запроса на удаление анкеты"""
...
async def send_delete_request(m: Message):
    """Отправка запроса на удаление анкеты администраторам"""
    # Проверяем, не отправлен ли уже запрос
...
async def change_maintainence(m: Message):
    """Включение/выключение режима технического обслуживания (только для администраторов)"""
...
async def restart(m: Message):
    """Перезапуск бота (только для администраторов в Linux)"""
...
async def timing(m: Message):
    """Открытие настроек таймеров (только для администраторов)"""
...
async def freeze_timing(m: Message):
    """Настройка времени до автоматической заморозки анкеты"""
...
async def set_freeze_timing(m: Message):
    """Установка времени до автоматической заморозки анкеты"""
    period = parse_period(m.text)
    if not period:
        return "Неверный период"

    # Запускаем проверку активности для всех пользователей
...
async def delete_timing(m: Message):
    """Настройка времени до автоматического удаления анкеты"""
...
async def set_delete_timing(m: Message):
    """Установка времени до автоматического удаления анкеты"""
    period = parse_period(m.text)
    if not period:
        return "Неверный период"

    # Проверяем, что время удаления больше времени заморозки
