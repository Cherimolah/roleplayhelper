...
async def group_join(event: GroupJoin):
    ...
    if await bot.api.groups.is_member(group_id, event.object.user_id):
        return
    await user.api.groups.approve_request(event.group_id, event.object.user_id)
    await asyncio.sleep(0.34)
    can_write = (await user.api.users.get(event.object.user_id, fields=["can_write_private_message"]))[0].can_write_private_message
    await asyncio.sleep(0.34)
    if can_write:
        await user.api.messages.send(user_id=event.object.user_id, random_id=0,
