...
Перед запуском бота рекомендуется сразу создать 2 топика (обсуждения) в группе.
1 топик - это доска активных анкет
2 топик - это архив анкет, куда будут публиковаться удаленные анкеты

Нужно получить их айди. Айди топика - последние числа в ссылке топика
...
**В переменные BOARD_FORMS_TOPIC_ID и ARCHIVE_FORMS_TOPIC_ID вставляем айди топиков**
...
После указания каюты пользователя бот спрашивает админа надо ли публиковать эту анкету в доску анкет
Если да, то бот автоматически загружает анкету в топик
...
Кратко про то, что добавлено поверх описанного выше. Код к каждой механике снабжён docstring'ами со ссылкой на соответствующий пункт ТЗ (`module ...` / `admin_improvements, п. ...`).

### Аукцион (`service/auctions.py`, `handlers/admin_panel/auctions.py`)

Судьи и админы выставляют на торги предметы Карты Экспедитора (`db.Item` — у него уже есть фото/описание и `fraction_id`/`reputation` для допуска). Мастер настройки: Меню -> Админ-панель / Панель судьи -> «Аукционы».

* **Публичный** — объявление постится на стену группы (арт берётся из карточки предмета), ставки принимаются комментариями к посту (число = сумма ставки, см. `handlers/group_events.py:wall_auction_bid`, требует включённых событий `wall_reply_new` в настройках Bots API сообщества). По истечении времени публикуется отдельный пост с победителем.
* **Закрытый** — таргетированная рассылка в ЛС только игрокам, проходящим по фракции/репутации предмета. Ставки — командой `[ставка СУММА]` в ЛС боту (`[ставка СУММА аукцион ID]`, если у игрока несколько активных закрытых аукционов сразу).

Ставка засчитывается, только если она не меньше текущей максимальной + `min_bid_step` (или стартовой цены, если ставок ещё не было).

### Режим от первого лица (POV) и дебафы

* Функции замазывания/перемешивания текста вынесены в `service/effects.py` (`limited_visibility`, `concussion`, `blindness`, `deafness` + реестр `EFFECTS` для новых эффектов).
* `service/pov_debuffs.py` связывает `db.StateDebuff.pov_effect` (настраивается в CRUD дебафов, как и остальные поля) с POV: выдача такого дебафа принудительно включает игроку POV на время действия дебафа и активирует эффект; снятие/истечение дебафа снимает эффект (но не сам POV — он выключается только вручную/администрацией).
* Административный инструмент **«POV режим»** (`handlers/admin_panel/blackout.py`) — принудительный перевод в POV отдельного игрока / всех игроков профессии / всех игроков, и снятие такого принудительного перевода. Пока перевод не снят администрацией, игрок не может выключить POV сам.
* **Скрытное действие** — `[скрытно "..."]` / `[скрытное действие "..."]` в чате-локации: сообщение сразу удаляется из чата, дальше автоматическая проверка Ловкость (автор) против лучшего Восприятия остальных участников локации. Провал — действие пересылается в чат как обычное. Успех — не показывается игрокам, логируется (`db.StealthLog`) и уходит только судьям/админам.

### Засекреченные данные анкет (module classified_profiles)

`db.Form` получил `secret_features/secret_bio/secret_character/secret_motives` и правило допуска (`classified_profession_id`, `classified_fraction_id` + `classified_reputation`). Владелец редактирует свои секреты через тот же список полей анкеты, что и остальные поля; админ дополнительно настраивает правило допуска через панель редактирования анкет. Для всех, кому просмотр не положен, секретные строки в `loads_form()` полностью отсутствуют в выводе (не редактируются — вырезаются).

### Административные доработки

* `[принудительное перемещение @user1 @user2]` — мгновенное перемещение (только админ/судья) в чат, где написана команда.
* Поиск анкет в админ-панели (`edit_form`) теперь показывает тот же нумерованный список, что и обычным игрокам, плюс старый поиск по упоминанию/ссылке.
* Назначение роли админа/судьи (`manage_admins.py`) выдаёт им права модератора («admin» через `messages.setMemberRole`) во всех зарегистрированных чатах.
* Удаление анкеты с каютой спрашивает администратора: удалить чат каюты (все выведены) или архивировать (переименован, в чат отправлено сообщение о бывшем жильце).
* «Массовый лимит сообщений в чатах» в админ-панели — меняет `visible_messages` сразу для всех зарегистрированных чатов.
* `[сообщения [id1|Name1], [id2|Name2] "текст"]` — рассылка одного сообщения нескольким адресатам; с флагом `засекречено` в конце команды доставляется только тем, у кого есть допуск к секретам отправителя (см. classified_profiles).
* Панель судьи получила «Принудительная выдача квестов» (существующий квест выдаётся выбранным игрокам напрямую).
* Назначение каюты новому игроку переиспользует уже существующий чат этого номера каюты (если он не был удалён), а не создаёт дубликат.
* `[выдать задачу @user1, @user2 "описание"]` и `[выдать задачу "описание"]` (судьи/админы/владелец) запускают мастер создания квеста (тот же, что и в «Изменение контента -> Квесты»); тегированный вариант принудительно выдаёт квест указанным игрокам сразу после создания, второй — оставляет квест доступным для добровольного взятия.